...
| 🧨 **Bot Danger Zone — Clear / Reset** | New collapsible "Danger Zone" panel on the Bot page. Four granular buttons: Reset Learning Model, Clear Activity Log, Clear Scan Logs, Clear Bot Trades (force-closes + returns cash). One-click "Full Reset" wipes everything (bot trades + portfolio + model + activity + scan logs) — manual forwardtest trades are always preserved. Every destructive action requires confirmation. |
| 💬 **Plain-English "Why" on every bot trade** | Bot trade notes are now a multi-paragraph story instead of a dense number dump. Tells you: what stage the stock is in (with plain meaning), why now (Wyckoff signals translated to English), how the Smart Money Score reads, whether news is a tailwind/headwind (with the actual smart-money entities named — "Trump / Buffett / Burry mentioned in a BUY context"), and the bot's rating + entry/stop/target plan. The dense numeric breakdown is still saved as `technical_notes` and shown behind a "Show technical breakdown" toggle. |
...
| `DELETE` | `/api/bot/activity` | Clear the bot decision log |
| `DELETE` | `/api/bot/scanlogs` | Delete every per-scan JSON detail file |
| `DELETE` | `/api/bot/trades?close_open=true` | Force-close + delete bot-sourced trades (cash returned to portfolio). Manual trades preserved |
| `POST` | `/api/bot/full-reset?reset_portfolio=true` | Nuclear option — wipes bot trades + portfolio + model + activity + scan logs in one call |
...
**Why split bot trade notes into a plain-English "story" + a separate technical breakdown?** The dense reasoning string (`score=0.706 tier=wyckoff+sms+news`, raw factor values, RSI/MACD numbers) is auditable but unreadable — users complained they couldn't tell *why* the bot picked a stock without staring at the breakdown. The new `_human_reasoning()` function tells a 4-paragraph story: stage with plain meaning → Wyckoff signals translated ("MACD just crossed positive — short-term momentum turned up") → Smart Money Score with a verbal band ("EXTREMELY bullish institutional alignment") → news context with the actual smart-money entities named ("Trump / Buffett mentioned in a BUY context — these names draw reflexive retail follow-through") → rating + plan with R/R. The dense version is kept on `technical_notes` and revealed by a "Show technical breakdown" toggle so power users can still audit the math.
